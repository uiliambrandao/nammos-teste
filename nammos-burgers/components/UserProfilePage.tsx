  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9] pb-10 font-sans relative">
      ...
      <main className="max-w-2xl mx-auto pt-24 px-4 sm:px-6 space-y-6 animate-fade-in-down">
        ...
        {/* 2.1 WALLET / CASHBACK CARD */}
        <section 
          onClick={() => navigate('/cashback')}
          className="bg-brand-black text-white p-6 rounded-[24px] shadow-lg shadow-gray-200 cursor-pointer relative overflow-hidden group transition-transform hover:scale-[1.01]"
        >
          <div className="relative z-10 flex justify-between items-center">
            <div>
               <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Minha Carteira</p>
               ...
               <p className="text-sm text-gray-400 mt-1 font-medium group-hover:text-white transition-colors">Ver extrato de cashback</p>
            </div>
            <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center text-white group-hover:bg-brand-orange group-hover:text-white transition-all">
          ...
          <div className="absolute -bottom-8 -left-8 w-24 h-24 bg-brand-orange/20 rounded-full blur-xl"></div>
        </section>
          ...
          <div className="flex items-start gap-4">
             ...
             {deliveryAddress ? (
               ...
               <div className="space-y-1">
                 ...
                 <button onClick={() => navigate('/addresses')} className="text-sm font-bold text-brand-orange underline decoration-orange-300 decoration-2 underline-offset-2">
                   Adicionar agora
                 </button>
               </div>
             )}
        ...
        <section className="bg-white p-6 rounded-[24px] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-50">
          ...
          <button 
            ...
            className="w-full mt-4 py-3 text-sm font-bold text-brand-orange border border-orange-100 rounded-xl hover:bg-orange-50 transition-colors"
          >
        ...
        <section className="bg-white rounded-[24px] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-50 overflow-hidden">
          <div className="p-2">
            {[
              { label: 'Meus Cupons', icon: 'tag', path: '/cupons' },
              { label: 'Indique e Ganhe', icon: 'gift', path: '/refer' },
              { label: 'Programa de Fidelidade', icon: 'star', path: '/loyalty' },
              { label: 'Notificações', icon: 'bell', path: '/notifications' },
              { label: 'Meus Endereços', icon: 'map', path: '/addresses' },
              { label: 'Ajuda e Suporte', icon: 'question', path: '/help' },
            ].map((item, i) => (
              <button 
                key={i} 
        ...
        <div className="pt-2 pb-8">
          ...
          <p className="text-center text-xs text-gray-300 mt-4">Versão 1.0.4</p>
        </div>
...
const SettingsIcon: React.FC<{ name: string }> = ({ name }) => {
  ...
  if (name === 'bell') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>;
  if (name === 'cog') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>;
  if (name === 'question') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
  if (name === 'shield') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>;
  if (name === 'tag') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>;
  if (name === 'gift') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" /></svg>;
  if (name === 'star') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>;
  if (name === 'map') return <svg xmlns="http://www.w3.org/2000/svg" className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>;
